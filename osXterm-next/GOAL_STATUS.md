...
- `Scripts/verify-terminal-catalog.sh` verified that all 20 declared themes have terminal-surface palettes, are exposed through Settings, and that the four bundled font choices are applied by the terminal adapter without a macOS system-font picker.
...
- All 59 Swift Testing core tests passed macro expansion and type checking with the Xcode Testing plugin, including broadcast source and recipient routing, disconnected-target pruning, concurrent-session title allocation, packaged-helper lookup, OpenSSH feature-baseline checks, and workspace persistence plus version rejection.
...
- A manually linked Swift Testing runner executed all 59 core tests successfully outside the seatbelt sandbox, including the real Unix-domain credential-broker IPC exchange, live loopback TCP and Unix socket destination probes, broadcast routing and disconnected-target pruning checks, concurrent-session title allocation, packaged-helper lookup, installed OpenSSH capability checks, atomically persisted workspace settings and layouts, version rejection, and staged session-log export checks for content, replacement cleanup, and mode 0600. This is supplemental direct-compiler evidence, not a substitute for the blocked `swift test` release gate.
- AskPass and proxy helpers are located only inside the app bundle or next to a SwiftPM development executable. If AskPass is absent, terminal, SFTP, SCP, and tunnel launch now fail before OpenSSH starts instead of silently omitting `SSH_ASKPASS`.
