...
- All 50 Swift Testing core tests passed macro expansion and type checking with the Xcode Testing plugin, including broadcast source and recipient routing rules plus OpenSSH feature-baseline checks.
...
- A manually linked Swift Testing runner executed all 50 core tests successfully outside the seatbelt sandbox, including the real Unix-domain credential-broker IPC exchange, live loopback TCP and Unix socket destination probes, broadcast routing checks, installed OpenSSH capability checks, and staged session-log export checks for content, replacement cleanup, and mode 0600. This is supplemental direct-compiler evidence, not a substitute for the blocked `swift test` release gate.
