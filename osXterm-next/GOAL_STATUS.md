...
- All 53 Swift Testing core tests passed macro expansion and type checking with the Xcode Testing plugin, including broadcast source and recipient routing, disconnected-target pruning, OpenSSH feature-baseline checks, and workspace persistence plus version rejection.
...
- A manually linked Swift Testing runner executed all 53 core tests successfully outside the seatbelt sandbox, including the real Unix-domain credential-broker IPC exchange, live loopback TCP and Unix socket destination probes, broadcast routing and disconnected-target pruning checks, installed OpenSSH capability checks, atomically persisted workspace settings and layouts, version rejection, and staged session-log export checks for content, replacement cleanup, and mode 0600. This is supplemental direct-compiler evidence, not a substitute for the blocked `swift test` release gate.
